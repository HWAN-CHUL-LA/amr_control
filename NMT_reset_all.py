...
def main():
    """
    설정된 모든 노드에 NMT Reset 명령을 전송합니다.
    """
    buses = {}
    try:
        ...
        print("\n" + "="*50)
        print("모든 노드에 NMT Reset 명령을 전송합니다...")
        print("="*50)
        ...
        for node_id, bus_name in NODE_CONFIG.items():
            ...
            # 데이터 페이로드: [명령어, 노드ID]
            data_payload = [CMD_RESET_NODE, node_id]
            ...
            try:
                bus.send(msg)
                print(f" -> Node {node_id} ({bus_name})에 Reset 명령 전송 완료.")
            except can.CanError as e:
                print(f" -> Node {node_id} ({bus_name})에 명령 전송 실패: {e}")
            
            time.sleep(0.05)
        
        print("\n모든 노드 리셋 명령 전송이 완료되었습니다.")
