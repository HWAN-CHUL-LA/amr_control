...
def send_sdo_write(
    ...
    with lock:
        ...
        deadline = time.time() + SDO_WRITE_ACK_TIMEOUT
        cob_id_resp = 0x580 + node_id
        while time.time() < deadline:
            remaining = deadline - time.time()
            response = BUS_POOL[bus_name].recv(timeout=remaining)
            if response is None:
                continue
            ...
            cmd = response.data[0]
            if cmd == 0x60:
                index_resp = response.data[1] | (response.data[2] << 8)
                sub_resp = response.data[3]
                if index_resp == index and sub_resp == sub_index:
                    return
                # Ignore stale acknowledgement frames from previous commands.
                continue
            if cmd == 0x80:
                index_resp = response.data[1] | (response.data[2] << 8)
                sub_resp = response.data[3]
                abort_code = struct.unpack("<I", bytes(response.data[4:8]))[0]
                raise RuntimeError(
                    f"SDO abort 0x{index_resp:04X}:{sub_resp} code 0x{abort_code:08X}"
                )
            raise RuntimeError(f"Unexpected SDO response 0x{cmd:02X}")
        raise TimeoutError(
            f"SDO write timeout 0x{index:04X}:{sub_index} on node {node_id}"
        )
...
def send_sdo_read(
    ...
    """
    Robust SDO upload (read). Ignores unrelated SDO responses (e.g., prior download acks 0x60)
    and only returns when we get an upload response (0x4x) that matches the requested index/subindex.
    """
    if bus_name not in BUS_POOL:
        raise RuntimeError(f"Bus '{bus_name}' not initialised.")
    ...
    payload = [0x40, index & 0xFF, (index >> 8) & 0xFF, sub_index, 0, 0, 0, 0]
    ...
    with lock:
        ...
        while time.time() < deadline:
            ...
            if response is None:
                continue
            if response.arbitration_id != cob_id_resp:
                # Not our SDO response; ignore
                continue

            cmd = response.data[0]
            idx_lo, idx_hi, sub = response.data[1], response.data[2], response.data[3]
            resp_index = (idx_hi << 8) | idx_lo

            # SDO abort
            if cmd == 0x80:
                abort_code = int.from_bytes(bytes(response.data[4:8]), byteorder="little", signed=False)
                raise RuntimeError(
                    f"SDO abort 0x{resp_index:04X}:{sub} while reading 0x{index:04X}:{sub_index} "
                    f"(code=0x{abort_code:08X})"
                )

            # Upload (read) initiate response family: 0x4F/0x4B/0x47/0x43 ...
            if (cmd & 0xE0) == 0x40:
                # Make sure this is the response to our request
                if resp_index != index or sub != sub_index:
                    # Different object → ignore and keep waiting
                    continue

                expedited = bool(cmd & 0x02)
                if not expedited:
                    # (Most drives report these as expedited; implement segmented only if ever needed)
                    raise RuntimeError("Segmented SDO uploads are not supported in this tool.")
            ...
            # Download (write) response 0x60 or other types → just ignore and continue
            # This is the key change that avoids raising on 0x60.
            continue
