...
def print_help(drive_nodes: List[int], steer_nodes: List[int]) -> None:
    with print_lock:
        ...
        print("  [F1/F2/F3] offset step 0.001° / 0.010° / 0.100°")
        print("  [<-]/[->] adjust selected offset by current step")
        print("  [p] print all offsets")
        print("  [s] save offsets to file     [l] load offsets from file")
        print("\nZero-turn keys:")
        print("  [z] align wheels for zero-radius rotation")
        print("  [x] restore wheels to 0°")
        print(f"  [,] rotate step -{ZERO_TURN_STEP_ANGLE_DEG:.1f}° (CW)")
        print(f"  [.] rotate step +{ZERO_TURN_STEP_ANGLE_DEG:.1f}° (CCW)")
...
def main() -> None:
    global drive_controller, steer_controller, is_running, selected_steer_node, zero_radius_helper
